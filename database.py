...
# --- Configuration ---
TOKEN = "8213242839:AAE4826Rx_O4Hqi3Op81kNv2vCbP2s67aHc"
SUDO_ID = 8150699034
...
async def sclaim(update: Update, context: CallbackContext):
    ...
    try:
        ...
        # New, requested message format for daily claim
        caption = (
...
async def drop_character(context: CallbackContext, chat_id: int, is_forced=False):
    ...
    caption = (
        f"🎨 {apply_font(f'ᴀ {rarity_name} ᴄʜᴀʀᴀᴄᴛᴇʀ ʜᴀs ᴀᴘᴘᴇᴀʀᴇᴅ!')}\n\n"
        f"🍥 {apply_font('ᴛʏᴘᴇ /grab ɴᴀᴍᴇ ᴛᴏ ᴄʟᴀɪᴍ ᴛʜᴇᴍ')}"
    )
...
async def select_character_by_rarity():
    ...
    try:
        ...
        # New logic: Lower spawn weight means higher chance
        ...
        # Calculate inverse weights, and make sure we don't divide by zero
        inverse_weights = [1.0 / weight if weight > 0 else 0 for weight in spawn_weights]
        
        # Use choices with the new inverse weights
        selected_rarity_name = random.choices(rarity_names, weights=inverse_weights, k=1)[0]
...
async def inline_query(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if match:
            ...
            query_str = "SELECT T1.name, T1.file_id, T1.is_video, T1.rarity, T1.anime_name, T2.emoji, T3.grab_count FROM characters T1 " \
                        "INNER JOIN rarities T2 ON T1.rarity = T2.name " \
                        "INNER JOIN user_harem T3 ON T1.character_id = T3.character_id " \
                        "WHERE T3.user_id = ?"
            ...
            for name, file_id, is_video, rarity, anime_name, emoji, grab_count in user_characters:
